...
class LaserTracker(object):
    ...
    def __init__(self, cam_width=1024, cam_height=720, hue_min=20, hue_max=160,
                 sat_min=50, sat_max=255, val_min=0, val_max=40,
    ...
    def setup_camera_capture(self, device_num=0):
       ...
       # cameraBackground = cv2.VideoCapture(int(device_num)).read()
        self.capture = cv2.VideoCapture(int(1))
        ''' while True:
            ...
            if timeNow - timeStart > 10:
                    #removeForeground in 5 seconds
                cameraBackground = cameraForeground(device)
                timeStart = timeNow'''
    ...
    def track(self, frame, mask):
        ...
        if len(countours) > 0:
            ...
            movemouse = [center[0] * mousefactor[0], center[1]* mousefactor[1]]
            
            timeNow = time.time()
            print(self.status)
            
              #  print(timeNow - self.timeStartMovement)
            deltaMousePosPast = [self.pastMousePos[0] - movemouse[0], self.pastMousePos[1] - movemouse[1] ]
            
            print(deltaMousePosPast)
            if(timeNow - self.timeStartMovement  > 0.1):
              pyautogui.mouseUp(button='left')
              self.status = "No movement"
              self.pastMousePos = movemouse
              self.timeStartMovement = time.time()
            elif(self.status == "drag"):
                if self.status != "drag":
                 pyautogui.mouseDown(button='left') 
                self.timeStartMovement = time.time() - 0
                self.status = "drag"
                deltaMousePosPast = [4,4]
            elif self.status == "click" or (float(timeNow) - float(self.timeStartMovement)>= 0.0 and  float(timeNow) - float(self.timeStartMovement)<= 0.1 and self.status == "No movement") : 
                ...
                self.status = "click"
                #transforma de click para drag se o mouse continuar a se mover dps do click
                if( deltaMousePosPast[0] > 40 or deltaMousePosPast[0] < -40  or  deltaMousePosPast[1] > 40 or  deltaMousePosPast[1] < -40):
                  self.status = "drag"  
                  pyautogui.click(button='left')
                  pyautogui.mouseDown(button='left') 
                  self.timeStartMovement = time.time() - 0.0
                self.timeStartMovement = time.time() - 0.0
